...
#SPD: modify for Android R by peng.zhang5 at 20200728 start
cat /proc/ion/ion_mm_heap | grep "  total orphaned" | tee -a $log_path/dumpsys_meminfo_$time.txt
cat /proc/ion/ion_mm_heap | grep "          total"  | tee -a $log_path/dumpsys_meminfo_$time.txt
#SPD: modify for Android R by peng.zhang5 at 20200728 end
curr_time=$(date +%s)
echo $curr_time
